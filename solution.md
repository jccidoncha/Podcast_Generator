...
| Audio      | Local FS in dev, S3-compatible in prod             | Local keeps dev frictionless; the path is abstracted via `StorageProvider` in `src/providers/storage.ts` so prod swap is one provider file.                                                                                                                                                                            |
...
- **Audio storage in S3** — local FS in dev. Production would write to S3-compatible storage; the `StorageProvider` abstraction in `src/providers/storage.ts` is already there (`LocalStorage` wired, `S3Storage` is a stub).
